...
const ResearchCard = ({ item, index }) => {
    ...
    return (
        <motion.div
            ...
            className="bg-neutral-900/30 rounded-3xl p-6 lg:p-10 border border-neutral-800/50 backdrop-blur-sm hover:border-neutral-700/50 transition-all shadow-2xl relative group"
        >
            {/* View Count Badge */}
            <div className="absolute top-6 right-6 bg-black/40 backdrop-blur-md border border-white/5 px-3 py-1 rounded-full flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <FaEye className="text-purple-400 text-xs" />
                <span className="text-neutral-300 text-[10px] font-medium tracking-wider uppercase">{formatViews(views)} Views</span>
            </div>
