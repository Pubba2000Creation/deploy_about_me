...
const ResearchImage = ({ src, title }) => (
    <div className="group relative overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900/50 hover:border-purple-500/50 transition-all duration-300 shadow-2xl">
        <img
            src={src}
            alt={title}
            className="w-full h-auto object-contain bg-neutral-950 group-hover:scale-[1.02] transition-transform duration-500"
        />
...
const ResearchDetails = () => {
    ...
    return (
        <div className="min-h-screen container mx-auto px-4 py-8 text-neutral-300">
            {/* Back Navigation */}
            <Link to="/" className="inline-flex items-center text-purple-400 hover:text-purple-300 transition-colors mb-8 group font-medium">
            ...
            >
                ...
                <div className="flex flex-wrap gap-3 mt-8">
                    {item.technologies.map((tech, index) => (
                        <span
                            key={index}
                            className="bg-neutral-900 border border-neutral-800 text-purple-300 px-4 py-1.5 rounded-full text-sm font-medium"
                        >
                            {tech}
                        </span>
                    ))}
            ...
            {item.papers && (
                <motion.div
                    ...
                    className="mb-20"
                >
                    <h2 className="text-2xl font-semibold text-white mb-8 border-l-4 border-red-500 pl-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {item.papers.map((paper, index) => (
                            <a
                                ...
                                className="flex items-center gap-6 p-6 bg-neutral-900/40 border border-neutral-800 rounded-2xl hover:bg-neutral-800/60 hover:border-red-500/30 transition-all group"
                            >
                                <div className="p-4 bg-red-500/10 rounded-xl group-hover:scale-110 transition-transform">
                                    <FaFilePdf className="text-4xl text-red-500" />
                                </div>
                                <div>
                                    <h3 className="text-xl font-medium text-white group-hover:text-red-400 transition-colors">
                                        {paper.title}
                                    </h3>
            ...
            {item.metrics && (
                <div className="space-y-24 pb-20">
                    ...
                    >
                        <h2 className="flex items-center gap-3 text-3xl font-light text-white mb-10">
